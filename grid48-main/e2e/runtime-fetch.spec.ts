...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'grid48.app',
          locationOrigin: 'https://grid48.app',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('grid48.app/api/fred-data')) {
        ...
        if (url.includes('grid48.app/api/stablecoin-markets')) {
    ...
    expect(result.calls.some((url) => url.includes('grid48.app/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('grid48.app/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('grid48.app/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('grid48.app/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('grid48.app/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('grid48.app/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/koala73/grid48/releases/latest';
    ...
    expect(result.macArm).toBe('https://grid48.app/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://grid48.app/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/koala73/grid48/releases/latest');
  });
  ...
  test('cloud fallback blocked without Grid48 API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('grid48.app'));
  ...
  test('cloud fallback allowed with valid Grid48 API key', async ({ page }) => {
    await page.goto('/tests/runtime-harness.html');
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('grid48.app') && init?.headers) {
          const h = new Headers(init.headers);
          const wmKey = h.get('X-Grid48-Key');
          if (wmKey) capturedHeaders['X-Grid48-Key'] = wmKey;
        }

        if (url.includes('127.0.0.1:46123/api/market/v1/test')) {
          throw new Error('ECONNREFUSED');
        }
        if (url.includes('grid48.app/api/market/v1/test')) {
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('grid48.app')).length,
          wmKeyHeader: capturedHeaders['X-Grid48-Key'] || null,
        };
      } finally {
        window.fetch = originalFetch;
        delete globalWindow.__wmFetchPatched;
