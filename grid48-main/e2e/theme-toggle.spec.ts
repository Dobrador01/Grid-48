...
test.describe('theme toggle (happy variant)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      if (!sessionStorage.getItem('__test_init_done')) {
        localStorage.removeItem('grid48-theme');
        localStorage.removeItem('grid48-variant');
        localStorage.setItem('grid48-variant', 'happy');
  ...
  test('dark mode persists across page reload', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('grid48-theme'));
  ...
  test('no FOUC: data-theme is set before main CSS loads', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('grid48-theme', 'dark');
      localStorage.setItem('grid48-variant', 'happy');
    });
