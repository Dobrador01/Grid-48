...
function parseMapaInfo(html: string): {
  nome: string;
  totalUcs: number;
  ucsSemEnergia: number;
} | null {
...
interface ParsedMunicipio {
  municipio: string; // normalized
  totalUcs: number;
  ucsAfetadas: number;
  bairrosAfetados: Array<{ nome: string; ucs: number }>;
}
...
export const pollCelesc = internalAction({
  ...
  handler: async (ctx): Promise<void> => {
    ...
    // ── Fetch A: mapa.js ──
    let mapaMunicipios: Map<string, { totalUcs: number; ucsAfetadas: number }> =
      new Map();
    ...
    const bairrosByMunicipio = new Map<
      string,
      Array<{ nome: string; ucs: number }>
    >();

    try {
      const tabelasRes = await fetch(
        "https://celgeoweb.celesc.com.br/json/tabelas.js"
      );
      ...
      if (Array.isArray(regionais)) {
        for (const regional of regionais) {
          ...
          for (const cidade of cidades) {
            ...
            const affected: Array<{ nome: string; ucs: number }> = [];
            for (const bairro of bairros) {
              const qty =
                Number(bairro.QUANTIDADE_ACIDENTAL) || 0;
              if (qty > 0) {
                affected.push({
      ...
      console.log(
        `[Celesc] Fetch B: found ${bairrosByMunicipio.size} municípios with affected bairros`
      );
    } catch (err) {
    ...
    const payloads: ParsedMunicipio[] = [];
    ...
    if (payloads.length > 0) {
      ...
      console.log(
        `[Celesc] Upserted ${payloads.length} municípios. Timestamp: ${timestamp}`
      );
    } else {
  ...
  args: {
    municipios: v.array(
      v.object({
        ...
        bairrosAfetados: v.array(
          v.object({ nome: v.string(), ucs: v.number() })
        ),
      })
    ),
    timestamp: v.string(),
