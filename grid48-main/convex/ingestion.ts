...
export const ingestRawAlert = action({
  ...
  handler: async (ctx, args): Promise<{ status: string; reason?: string; alertId?: any }> => {
    ...
    const geminiResult: any = await ctx.runAction(api.gemini.processAlert, {
      text: args.text,
    });

    // Salva o alerta estruturado no banco de dados
    const alertId: any = await ctx.runMutation(api.ingestion.saveAlert, {
