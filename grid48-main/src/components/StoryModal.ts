...
function downloadStory(): void {
  ...
  a.download = `grid48-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-grid48.png`, { type: 'image/png' });
