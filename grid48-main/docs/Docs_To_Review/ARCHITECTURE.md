...
| **IndexedDB** | `grid48_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
Grid 48 ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | grid48.app | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.grid48.app | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.grid48.app | Markets, trading, central banks, macro indicators |
...
```
localStorage('grid48-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
export const SITE_VARIANT: string = (() => {
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('grid48-variant');
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(grid48-layers)"]
...
2. **User localStorage** — Stored under the key `grid48-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
Grid 48 employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: grid48-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `grid48_db` IndexedDB database contains two object stores:
...
On desktop, `getPersistentCache()` and `setPersistentCache()` attempt Tauri IPC invocations (`read_cache_entry` / `write_cache_entry`) first, which store data on the OS filesystem via the Rust backend. If the Tauri call fails (or in web mode), the module falls back to `localStorage` with the prefix `grid48-persistent-cache:`.
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `grid48-runtime-feature-toggles`:
...
Grid 48 runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
Grid 48 uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
