# Contributing to Grid 48

Thank you for your interest in contributing to Grid 48! This project thrives on community contributions — whether it's code, data sources, documentation, or bug reports.
...
Grid 48 is a real-time OSINT dashboard built with **Vanilla TypeScript** (no UI framework), **MapLibre GL + deck.gl** for map rendering, and a custom Proto-first RPC framework called **Sebuf** for all API communication.
...
2. **Clone** your fork locally:
   ```bash
   git clone https://github.com/<your-username>/grid48.git
   cd grid48
   ```
...
- **Internationalization** — help make Grid 48 available in more languages
...
- Sebuf handler implementations go in `server/grid48/{domain}/v1/`
...
- Proto definitions go in `proto/grid48/{domain}/v1/`
...
1. **Proto definitions** in `proto/grid48/{domain}/v1/` define services and messages
...
3. **Handlers** in `server/grid48/{domain}/v1/handler.ts` implement the service interface
...
2. **Add the proto service** (if the data needs a backend proxy) — define messages and RPC methods in `proto/grid48/{domain}/v1/`
3. **Generate stubs** — run `make generate`
4. **Implement the handler** in `server/grid48/{domain}/v1/`
...
rclone copy public/data/country-boundary-overrides.geojson r2:grid48-maps/
```
...
Use the [Bug Report issue template](https://github.com/koala73/grid48/issues/new/choose) when available.
...
Use the [Feature Request issue template](https://github.com/koala73/grid48/issues/new/choose) when available.
...
Thank you for helping make Grid 48 better! 🌍
